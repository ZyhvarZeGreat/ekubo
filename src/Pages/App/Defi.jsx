import React from "react";
import "./Defi.css";
const Defi = () => {
    return (
        <div className="chakra-container css-e3bxc">
            <p className="chakra-text  css-1rz2glp">Starknet DeFi Spring</p>
            <div className="css-ch4vff">
                <p className="chakra-text css-zfx5r2">90M</p>
                <svg
                    viewBox="0 0 111 110"
                    focusable="false"
                    className="chakra-icon css-onkibi"
                >
                    <path
                        d="M2.11688 54.999C2.11688 84.4816 26.0167 108.382 55.4992 108.382C84.9818 108.382 108.884 84.4815 108.884 54.999C108.884 25.5166 84.9818 1.6167 55.4992 1.6167C26.0167 1.6167 2.11688 25.5165 2.11688 54.999Z"
                        fill="#0C0C4F"
                        stroke="#EC796B"
                        strokeWidth="3.23374"
                    ></path>
                    <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M31.2402 42.042L32.5979 37.8475C32.8738 36.9944 33.5471 36.3305 34.4035 36.0688L38.6184 34.7726C39.2019 34.5944 39.2066 33.7713 38.6279 33.5836L34.4318 32.2259C33.5803 31.9499 32.9164 31.2766 32.6531 30.4204L31.3584 26.2054C31.1803 25.6235 30.3571 25.6172 30.1695 26.1975L28.8118 30.392C28.5359 31.2435 27.8626 31.9073 27.0062 32.1707L22.7913 33.4653C22.2078 33.6452 22.2015 34.4667 22.7818 34.6544L26.9779 36.012C27.8294 36.2879 28.4933 36.9629 28.7566 37.8191L30.0512 42.0326C30.2294 42.616 31.0526 42.6223 31.2402 42.042Z"
                        fill="#FAFAFA"
                    ></path>
                    <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M97.8629 39.6004C96.1253 37.6573 93.4129 36.5633 90.7764 36.1146C88.1187 35.6837 85.3324 35.7237 82.7086 36.1897C77.4002 37.0877 72.5776 39.2858 68.3713 41.9975C66.187 43.3283 64.3243 44.8681 62.3944 46.4363C61.4645 47.2294 60.6168 48.0741 59.7339 48.9066L57.3213 51.307C54.7 54.047 52.1162 56.5428 49.6163 58.6114C47.1062 60.6705 44.7596 62.2344 42.4469 63.3265C40.1357 64.4242 37.6627 65.0699 34.4395 65.1732C31.2444 65.2858 27.4643 64.7093 23.4209 63.7574C19.3558 62.8098 15.0869 61.4592 10.3167 60.2969C11.9811 64.9146 14.4876 68.9952 17.7057 72.7254C20.9614 76.3908 25.0269 79.7317 30.2496 81.9289C35.3968 84.1747 41.8652 84.9808 47.9134 83.7645C53.9776 82.5973 59.2993 79.7912 63.6008 76.5459C67.9134 73.2673 71.4024 69.5328 74.3439 65.6502C75.156 64.5775 75.5854 63.9771 76.173 63.1386L77.7969 60.733C78.9258 59.2449 79.9531 57.5506 81.0703 56.0762C83.2605 52.9884 85.4198 49.9042 87.9292 47.0627C89.1921 45.6215 90.5234 44.2429 92.0958 42.9182C92.8799 42.2715 93.7268 41.6386 94.6655 41.0686C95.6182 40.4537 96.624 39.9724 97.8629 39.6004Z"
                        fill="url(#paint0_linear_4664_62620)"
                    ></path>
                    <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M97.8629 39.6004C95.9957 34.89 92.5253 30.9248 87.8673 27.9989C83.2375 25.105 76.807 23.6282 70.4351 24.8871C67.2871 25.4959 64.2373 26.6681 61.561 28.2289C58.8967 29.7837 56.5093 31.6552 54.4425 33.6629C53.4107 34.6703 52.4741 35.721 51.5437 36.7776L49.1323 39.8521L45.4077 44.8014C40.6594 51.1691 35.5461 58.6317 27.1549 60.8429C18.9172 63.0137 15.3444 61.0912 10.3167 60.2969C11.2359 62.6704 12.3747 64.9754 13.9185 67.003C15.4334 69.0718 17.2229 71.0149 19.4478 72.6805C20.572 73.4804 21.7591 74.2699 23.0774 74.9395C24.3898 75.5864 25.805 76.1558 27.3157 76.5846C30.3204 77.4081 33.7034 77.6966 36.9788 77.2536C40.2558 76.8164 43.388 75.7778 46.1258 74.3986C48.884 73.0324 51.2865 71.3685 53.4319 69.6125C57.6964 66.071 61.0136 62.1579 63.8156 58.2024C65.225 56.2248 66.5043 54.2098 67.6875 52.1943L69.0801 49.7948C69.5059 49.0934 69.9365 48.3877 70.3739 47.7304C72.1383 45.0897 73.8642 42.9724 75.9605 41.3831C78.0279 39.7527 80.9062 38.5482 84.7529 38.2683C88.5836 37.9851 93.0058 38.5083 97.8629 39.6004Z"
                        fill="#FAFAFA"
                    ></path>
                    <path
                        fillRule="evenodd"
                        clipRule="evenodd"
                        d="M77.1387 78.7385C77.1387 82.1982 79.9448 85.0043 83.4046 85.0043C86.8643 85.0043 89.667 82.1982 89.667 78.7385C89.667 75.2788 86.8643 72.4727 83.4046 72.4727C79.9448 72.4727 77.1387 75.2788 77.1387 78.7385Z"
                        fill="url(#paint1_linear_4664_62620)"
                    ></path>
                    <defs>
                        <linearGradient
                            id="paint0_linear_4664_62620"
                            x1="97.394"
                            y1="37.9475"
                            x2="-45.6771"
                            y2="112.385"
                            gradientUnits="userSpaceOnUse"
                        >
                            <stop stopColor="#EC796B"></stop>
                            <stop offset="1" stopColor="#D672EF"></stop>
                        </linearGradient>
                        <linearGradient
                            id="paint1_linear_4664_62620"
                            x1="89.6626"
                            y1="74.3429"
                            x2="66.8299"
                            y2="76.4834"
                            gradientUnits="userSpaceOnUse"
                        >
                            <stop stopColor="#EC796B"></stop>
                            <stop offset="1" stopColor="#D672EF"></stop>
                        </linearGradient>
                    </defs>
                </svg>
                <p className="chakra-text css-zfx5r2">STRK</p>
            </div>
            <p className="chakra-text css-1a0rqt9">
                Ekubo Protocol users will receive STRK incentives as part of the
                <a
                    target="_blank"
                    rel="noopener noreferer"
                    className="chakra-link css-1f0ul0p"
                    href="https://docs.ekubo.org/about-ekubo/starknet-defi-spring"
                >
                    Starknet DeFi Spring
                </a>
                Program.
            </p>
            <div className="css-f400xj">
                <p className="chakra-text css-3ridzu">Add liquidity</p>
                <p className="chakra-text css-2odlu9">
                    to one of these pairs to participate.
                </p>
            </div>
            <div className="css-19zvcq">
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=USDT&amp;quoteCurrency=USDC"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/c8a721d1-07c3-46e4-ab4e-523977c30b00/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e5aaa970-a998-47e8-bd43-4a3b56b87200/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    USDC&nbsp;-&nbsp;USDT
                                </span>
                                <p className="chakra-text css-2axgst">+11.47% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=STRK&amp;quoteCurrency=ETH"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e07829b7-0382-4e03-7ecd-a478c5aa9f00/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/1b126320-367c-48ed-cf5a-ba7580e49600/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    STRK&nbsp;-&nbsp;ETH
                                </span>
                                <p className="chakra-text css-2axgst">+34.78% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=ETH&amp;quoteCurrency=USDC"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e5aaa970-a998-47e8-bd43-4a3b56b87200/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e07829b7-0382-4e03-7ecd-a478c5aa9f00/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    ETH&nbsp;-&nbsp;USDC
                                </span>
                                <p className="chakra-text css-2axgst">+28.92% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=STRK&amp;quoteCurrency=USDC"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e5aaa970-a998-47e8-bd43-4a3b56b87200/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/1b126320-367c-48ed-cf5a-ba7580e49600/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    STRK&nbsp;-&nbsp;USDC
                                </span>
                                <p className="chakra-text css-2axgst">+33.43% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=WBTC&amp;quoteCurrency=ETH"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e07829b7-0382-4e03-7ecd-a478c5aa9f00/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/7dcb2db2-a7a7-44af-660b-8262e057a100/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    WBTC&nbsp;-&nbsp;ETH
                                </span>
                                <p className="chakra-text css-2axgst">+27.8% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=wstETH&amp;quoteCurrency=ETH"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e07829b7-0382-4e03-7ecd-a478c5aa9f00/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/dbbcbdea-1a92-437d-3701-4a5ee129d000/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    wstETH&nbsp;-&nbsp;ETH
                                </span>
                                <p className="chakra-text css-2axgst">+9.41% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
                <div className="css-0">
                    <a
                        className="chakra-card css-1eu6rwr"
                        href="/positions/new?baseCurrency=DAI&amp;quoteCurrency=USDC"
                    >
                        <div className="css-k008qs">
                            <div role="group" className="chakra-avatar__group css-rmccqp">
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/919e761b-56f7-4f53-32aa-5e066f7f6200/logo"
                                    className="chakra-image css-10dqpmn"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                                <img
                                    src="https://imagedelivery.net/0xPAQaDtnQhBs8IzYRIlNg/e5aaa970-a998-47e8-bd43-4a3b56b87200/logo"
                                    className="chakra-image css-ceshos"
                                    style={{
                                        border: "2px solid rgb(255, 255, 255)",
                                        overflow: "hidden",
                                        borderRadius: "50%",
                                    }}
                                />
                            </div>
                            <div className="css-0">
                                <span className="chakra-text css-s4urue">
                                    USDC&nbsp;-&nbsp;DAI
                                </span>
                                <p className="chakra-text css-2axgst">+16.98% APR</p>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
            <button type="button" className="chakra-button css-3obd69">
                <div className="css-1lads1q">
                    <div className="css-15axsv5">
                        <p className="chakra-text css-1eoz3uu">Connect wallet to claim</p>
                    </div>
                </div>
            </button>
            <div className="chakra-card css-v3lm4n">
                <div className="chakra-card__header css-1sl53ol">
                    <div className="css-gmuwbf">
                        <span className="chakra-text css-19i88g1">
                            Next claim available on
                            <span className="chakra-text css-1nu1n91">October 04</span>
                        </span>
                    </div>
                </div>
                <hr
                    aria-orientation="horizontal"
                    className="chakra-divider css-svjswr"
                />
                <div className="chakra-card__body css-1idwstw">
                    <div className="css-k008qs">
                        <div className="css-1k87v8c">
                            <div className="css-rklm6r">
                                <button type="button" className="chakra-button css-71oh5q">
                                    Show all
                                </button>
                            </div>
                            <div className="css-0">
                                <div className="css-gmuwbf">Period</div>
                            </div>
                            <div className="css-0"></div>
                            <div className="css-0">
                                <div className="css-gmuwbf">Status</div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-6xwb09">
                                    Sep 12 through Sep 19
                                </div>
                                <div
                                    id="popover-trigger-:r3:"
                                    aria-haspopup="dialog"
                                    aria-expanded="false"
                                    aria-controls="popover-content-:r3:"
                                    className="css-1y4civq"
                                >
                                    468,519.47
                                    <svg
                                        viewBox="0 0 111 110"
                                        focusable="false"
                                        className="chakra-icon css-1q2a7ix"
                                    >
                                        <path
                                            d="M2.11688 54.999C2.11688 84.4816 26.0167 108.382 55.4992 108.382C84.9818 108.382 108.884 84.4815 108.884 54.999C108.884 25.5166 84.9818 1.6167 55.4992 1.6167C26.0167 1.6167 2.11688 25.5165 2.11688 54.999Z"
                                            fill="#0C0C4F"
                                            stroke="#EC796B"
                                            strokeWidth="3.23374"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M31.2402 42.042L32.5979 37.8475C32.8738 36.9944 33.5471 36.3305 34.4035 36.0688L38.6184 34.7726C39.2019 34.5944 39.2066 33.7713 38.6279 33.5836L34.4318 32.2259C33.5803 31.9499 32.9164 31.2766 32.6531 30.4204L31.3584 26.2054C31.1803 25.6235 30.3571 25.6172 30.1695 26.1975L28.8118 30.392C28.5359 31.2435 27.8626 31.9073 27.0062 32.1707L22.7913 33.4653C22.2078 33.6452 22.2015 34.4667 22.7818 34.6544L26.9779 36.012C27.8294 36.2879 28.4933 36.9629 28.7566 37.8191L30.0512 42.0326C30.2294 42.616 31.0526 42.6223 31.2402 42.042Z"
                                            fill="#FAFAFA"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M97.8629 39.6004C96.1253 37.6573 93.4129 36.5633 90.7764 36.1146C88.1187 35.6837 85.3324 35.7237 82.7086 36.1897C77.4002 37.0877 72.5776 39.2858 68.3713 41.9975C66.187 43.3283 64.3243 44.8681 62.3944 46.4363C61.4645 47.2294 60.6168 48.0741 59.7339 48.9066L57.3213 51.307C54.7 54.047 52.1162 56.5428 49.6163 58.6114C47.1062 60.6705 44.7596 62.2344 42.4469 63.3265C40.1357 64.4242 37.6627 65.0699 34.4395 65.1732C31.2444 65.2858 27.4643 64.7093 23.4209 63.7574C19.3558 62.8098 15.0869 61.4592 10.3167 60.2969C11.9811 64.9146 14.4876 68.9952 17.7057 72.7254C20.9614 76.3908 25.0269 79.7317 30.2496 81.9289C35.3968 84.1747 41.8652 84.9808 47.9134 83.7645C53.9776 82.5973 59.2993 79.7912 63.6008 76.5459C67.9134 73.2673 71.4024 69.5328 74.3439 65.6502C75.156 64.5775 75.5854 63.9771 76.173 63.1386L77.7969 60.733C78.9258 59.2449 79.9531 57.5506 81.0703 56.0762C83.2605 52.9884 85.4198 49.9042 87.9292 47.0627C89.1921 45.6215 90.5234 44.2429 92.0958 42.9182C92.8799 42.2715 93.7268 41.6386 94.6655 41.0686C95.6182 40.4537 96.624 39.9724 97.8629 39.6004Z"
                                            fill="url(#paint0_linear_4664_62620)"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M97.8629 39.6004C95.9957 34.89 92.5253 30.9248 87.8673 27.9989C83.2375 25.105 76.807 23.6282 70.4351 24.8871C67.2871 25.4959 64.2373 26.6681 61.561 28.2289C58.8967 29.7837 56.5093 31.6552 54.4425 33.6629C53.4107 34.6703 52.4741 35.721 51.5437 36.7776L49.1323 39.8521L45.4077 44.8014C40.6594 51.1691 35.5461 58.6317 27.1549 60.8429C18.9172 63.0137 15.3444 61.0912 10.3167 60.2969C11.2359 62.6704 12.3747 64.9754 13.9185 67.003C15.4334 69.0718 17.2229 71.0149 19.4478 72.6805C20.572 73.4804 21.7591 74.2699 23.0774 74.9395C24.3898 75.5864 25.805 76.1558 27.3157 76.5846C30.3204 77.4081 33.7034 77.6966 36.9788 77.2536C40.2558 76.8164 43.388 75.7778 46.1258 74.3986C48.884 73.0324 51.2865 71.3685 53.4319 69.6125C57.6964 66.071 61.0136 62.1579 63.8156 58.2024C65.225 56.2248 66.5043 54.2098 67.6875 52.1943L69.0801 49.7948C69.5059 49.0934 69.9365 48.3877 70.3739 47.7304C72.1383 45.0897 73.8642 42.9724 75.9605 41.3831C78.0279 39.7527 80.9062 38.5482 84.7529 38.2683C88.5836 37.9851 93.0058 38.5083 97.8629 39.6004Z"
                                            fill="#FAFAFA"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M77.1387 78.7385C77.1387 82.1982 79.9448 85.0043 83.4046 85.0043C86.8643 85.0043 89.667 82.1982 89.667 78.7385C89.667 75.2788 86.8643 72.4727 83.4046 72.4727C79.9448 72.4727 77.1387 75.2788 77.1387 78.7385Z"
                                            fill="url(#paint1_linear_4664_62620)"
                                        ></path>
                                        <defs>
                                            <linearGradient
                                                id="paint0_linear_4664_62620"
                                                x1="97.394"
                                                y1="37.9475"
                                                x2="-45.6771"
                                                y2="112.385"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop stopColor="#EC796B"></stop>
                                                <stop offset="1" stopColor="#D672EF"></stop>
                                            </linearGradient>
                                            <linearGradient
                                                id="paint1_linear_4664_62620"
                                                x1="89.6626"
                                                y1="74.3429"
                                                x2="66.8299"
                                                y2="76.4834"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop stopColor="#EC796B"></stop>
                                                <stop offset="1" stopColor="#D672EF"></stop>
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </div>
                            </div>
                            <div

                                className="css-8hn81s">
                                <div className="css-p5pt5c">
                                    <div
                                        aria-valuemax="100"
                                        aria-valuemin="0"
                                        aria-valuenow="100"
                                        role="progressbar"
                                        className="css-1g2dxwf"
                                        style={{ width: "100%" }}
                                    ></div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-ryjd5e">
                                    <div className="css-0">
                                        <div
                                            id="popover-trigger-:r5:"
                                            aria-haspopup="dialog"
                                            aria-expanded="false"
                                            aria-controls="popover-content-:r5:"
                                            className="css-gmuwbf"
                                        >
                                            Completed
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-6xwb09">
                                    Sep 19 through Sep 26
                                </div>
                                <div
                                    id="popover-trigger-:r7:"
                                    aria-haspopup="dialog"
                                    aria-expanded="false"
                                    aria-controls="popover-content-:r7:"
                                    className="css-1y4civq"
                                >
                                    373,368.94
                                    <svg
                                        viewBox="0 0 111 110"
                                        focusable="false"
                                        className="chakra-icon css-1q2a7ix"
                                    >
                                        <path
                                            d="M2.11688 54.999C2.11688 84.4816 26.0167 108.382 55.4992 108.382C84.9818 108.382 108.884 84.4815 108.884 54.999C108.884 25.5166 84.9818 1.6167 55.4992 1.6167C26.0167 1.6167 2.11688 25.5165 2.11688 54.999Z"
                                            fill="#0C0C4F"
                                            stroke="#EC796B"
                                            strokeWidth="3.23374"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M31.2402 42.042L32.5979 37.8475C32.8738 36.9944 33.5471 36.3305 34.4035 36.0688L38.6184 34.7726C39.2019 34.5944 39.2066 33.7713 38.6279 33.5836L34.4318 32.2259C33.5803 31.9499 32.9164 31.2766 32.6531 30.4204L31.3584 26.2054C31.1803 25.6235 30.3571 25.6172 30.1695 26.1975L28.8118 30.392C28.5359 31.2435 27.8626 31.9073 27.0062 32.1707L22.7913 33.4653C22.2078 33.6452 22.2015 34.4667 22.7818 34.6544L26.9779 36.012C27.8294 36.2879 28.4933 36.9629 28.7566 37.8191L30.0512 42.0326C30.2294 42.616 31.0526 42.6223 31.2402 42.042Z"
                                            fill="#FAFAFA"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M97.8629 39.6004C96.1253 37.6573 93.4129 36.5633 90.7764 36.1146C88.1187 35.6837 85.3324 35.7237 82.7086 36.1897C77.4002 37.0877 72.5776 39.2858 68.3713 41.9975C66.187 43.3283 64.3243 44.8681 62.3944 46.4363C61.4645 47.2294 60.6168 48.0741 59.7339 48.9066L57.3213 51.307C54.7 54.047 52.1162 56.5428 49.6163 58.6114C47.1062 60.6705 44.7596 62.2344 42.4469 63.3265C40.1357 64.4242 37.6627 65.0699 34.4395 65.1732C31.2444 65.2858 27.4643 64.7093 23.4209 63.7574C19.3558 62.8098 15.0869 61.4592 10.3167 60.2969C11.9811 64.9146 14.4876 68.9952 17.7057 72.7254C20.9614 76.3908 25.0269 79.7317 30.2496 81.9289C35.3968 84.1747 41.8652 84.9808 47.9134 83.7645C53.9776 82.5973 59.2993 79.7912 63.6008 76.5459C67.9134 73.2673 71.4024 69.5328 74.3439 65.6502C75.156 64.5775 75.5854 63.9771 76.173 63.1386L77.7969 60.733C78.9258 59.2449 79.9531 57.5506 81.0703 56.0762C83.2605 52.9884 85.4198 49.9042 87.9292 47.0627C89.1921 45.6215 90.5234 44.2429 92.0958 42.9182C92.8799 42.2715 93.7268 41.6386 94.6655 41.0686C95.6182 40.4537 96.624 39.9724 97.8629 39.6004Z"
                                            fill="url(#paint0_linear_4664_62620)"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M97.8629 39.6004C95.9957 34.89 92.5253 30.9248 87.8673 27.9989C83.2375 25.105 76.807 23.6282 70.4351 24.8871C67.2871 25.4959 64.2373 26.6681 61.561 28.2289C58.8967 29.7837 56.5093 31.6552 54.4425 33.6629C53.4107 34.6703 52.4741 35.721 51.5437 36.7776L49.1323 39.8521L45.4077 44.8014C40.6594 51.1691 35.5461 58.6317 27.1549 60.8429C18.9172 63.0137 15.3444 61.0912 10.3167 60.2969C11.2359 62.6704 12.3747 64.9754 13.9185 67.003C15.4334 69.0718 17.2229 71.0149 19.4478 72.6805C20.572 73.4804 21.7591 74.2699 23.0774 74.9395C24.3898 75.5864 25.805 76.1558 27.3157 76.5846C30.3204 77.4081 33.7034 77.6966 36.9788 77.2536C40.2558 76.8164 43.388 75.7778 46.1258 74.3986C48.884 73.0324 51.2865 71.3685 53.4319 69.6125C57.6964 66.071 61.0136 62.1579 63.8156 58.2024C65.225 56.2248 66.5043 54.2098 67.6875 52.1943L69.0801 49.7948C69.5059 49.0934 69.9365 48.3877 70.3739 47.7304C72.1383 45.0897 73.8642 42.9724 75.9605 41.3831C78.0279 39.7527 80.9062 38.5482 84.7529 38.2683C88.5836 37.9851 93.0058 38.5083 97.8629 39.6004Z"
                                            fill="#FAFAFA"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M77.1387 78.7385C77.1387 82.1982 79.9448 85.0043 83.4046 85.0043C86.8643 85.0043 89.667 82.1982 89.667 78.7385C89.667 75.2788 86.8643 72.4727 83.4046 72.4727C79.9448 72.4727 77.1387 75.2788 77.1387 78.7385Z"
                                            fill="url(#paint1_linear_4664_62620)"
                                        ></path>
                                        <defs>
                                            <linearGradient
                                                id="paint0_linear_4664_62620"
                                                x1="97.394"
                                                y1="37.9475"
                                                x2="-45.6771"
                                                y2="112.385"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop stopColor="#EC796B"></stop>
                                                <stop offset="1" stopColor="#D672EF"></stop>
                                            </linearGradient>
                                            <linearGradient
                                                id="paint1_linear_4664_62620"
                                                x1="89.6626"
                                                y1="74.3429"
                                                x2="66.8299"
                                                y2="76.4834"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop stopColor="#EC796B"></stop>
                                                <stop offset="1" stopColor="#D672EF"></stop>
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </div>
                            </div>
                            <div className="css-8hn81s">
                                <div className="css-p5pt5c">
                                    <div
                                        aria-valuemax="100"
                                        aria-valuemin="0"
                                        aria-valuenow="100"
                                        role="progressbar"
                                        className="css-1g2dxwf"
                                        style={{ width: "100%" }}
                                    ></div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-ryjd5e">
                                    <div className="css-0">
                                        <div
                                            id="popover-trigger-:r9:"
                                            aria-haspopup="dialog"
                                            aria-expanded="false"
                                            aria-controls="popover-content-:r9:"
                                            className="css-gmuwbf"
                                        >
                                            Completed
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-jabydh">
                                    Sep 26 through Oct 03
                                </div>
                                <div
                                    id="popover-trigger-:rb:"
                                    aria-haspopup="dialog"
                                    aria-expanded="false"
                                    aria-controls="popover-content-:rb:"
                                    className="css-1y4civq"
                                >
                                    46,946.23
                                    <svg
                                        viewBox="0 0 111 110"
                                        focusable="false"
                                        className="chakra-icon css-1q2a7ix"
                                    >
                                        <path
                                            d="M2.11688 54.999C2.11688 84.4816 26.0167 108.382 55.4992 108.382C84.9818 108.382 108.884 84.4815 108.884 54.999C108.884 25.5166 84.9818 1.6167 55.4992 1.6167C26.0167 1.6167 2.11688 25.5165 2.11688 54.999Z"
                                            fill="#0C0C4F"
                                            stroke="#EC796B"
                                            strokeWidth="3.23374"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M31.2402 42.042L32.5979 37.8475C32.8738 36.9944 33.5471 36.3305 34.4035 36.0688L38.6184 34.7726C39.2019 34.5944 39.2066 33.7713 38.6279 33.5836L34.4318 32.2259C33.5803 31.9499 32.9164 31.2766 32.6531 30.4204L31.3584 26.2054C31.1803 25.6235 30.3571 25.6172 30.1695 26.1975L28.8118 30.392C28.5359 31.2435 27.8626 31.9073 27.0062 32.1707L22.7913 33.4653C22.2078 33.6452 22.2015 34.4667 22.7818 34.6544L26.9779 36.012C27.8294 36.2879 28.4933 36.9629 28.7566 37.8191L30.0512 42.0326C30.2294 42.616 31.0526 42.6223 31.2402 42.042Z"
                                            fill="#FAFAFA"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M97.8629 39.6004C96.1253 37.6573 93.4129 36.5633 90.7764 36.1146C88.1187 35.6837 85.3324 35.7237 82.7086 36.1897C77.4002 37.0877 72.5776 39.2858 68.3713 41.9975C66.187 43.3283 64.3243 44.8681 62.3944 46.4363C61.4645 47.2294 60.6168 48.0741 59.7339 48.9066L57.3213 51.307C54.7 54.047 52.1162 56.5428 49.6163 58.6114C47.1062 60.6705 44.7596 62.2344 42.4469 63.3265C40.1357 64.4242 37.6627 65.0699 34.4395 65.1732C31.2444 65.2858 27.4643 64.7093 23.4209 63.7574C19.3558 62.8098 15.0869 61.4592 10.3167 60.2969C11.9811 64.9146 14.4876 68.9952 17.7057 72.7254C20.9614 76.3908 25.0269 79.7317 30.2496 81.9289C35.3968 84.1747 41.8652 84.9808 47.9134 83.7645C53.9776 82.5973 59.2993 79.7912 63.6008 76.5459C67.9134 73.2673 71.4024 69.5328 74.3439 65.6502C75.156 64.5775 75.5854 63.9771 76.173 63.1386L77.7969 60.733C78.9258 59.2449 79.9531 57.5506 81.0703 56.0762C83.2605 52.9884 85.4198 49.9042 87.9292 47.0627C89.1921 45.6215 90.5234 44.2429 92.0958 42.9182C92.8799 42.2715 93.7268 41.6386 94.6655 41.0686C95.6182 40.4537 96.624 39.9724 97.8629 39.6004Z"
                                            fill="url(#paint0_linear_4664_62620)"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M97.8629 39.6004C95.9957 34.89 92.5253 30.9248 87.8673 27.9989C83.2375 25.105 76.807 23.6282 70.4351 24.8871C67.2871 25.4959 64.2373 26.6681 61.561 28.2289C58.8967 29.7837 56.5093 31.6552 54.4425 33.6629C53.4107 34.6703 52.4741 35.721 51.5437 36.7776L49.1323 39.8521L45.4077 44.8014C40.6594 51.1691 35.5461 58.6317 27.1549 60.8429C18.9172 63.0137 15.3444 61.0912 10.3167 60.2969C11.2359 62.6704 12.3747 64.9754 13.9185 67.003C15.4334 69.0718 17.2229 71.0149 19.4478 72.6805C20.572 73.4804 21.7591 74.2699 23.0774 74.9395C24.3898 75.5864 25.805 76.1558 27.3157 76.5846C30.3204 77.4081 33.7034 77.6966 36.9788 77.2536C40.2558 76.8164 43.388 75.7778 46.1258 74.3986C48.884 73.0324 51.2865 71.3685 53.4319 69.6125C57.6964 66.071 61.0136 62.1579 63.8156 58.2024C65.225 56.2248 66.5043 54.2098 67.6875 52.1943L69.0801 49.7948C69.5059 49.0934 69.9365 48.3877 70.3739 47.7304C72.1383 45.0897 73.8642 42.9724 75.9605 41.3831C78.0279 39.7527 80.9062 38.5482 84.7529 38.2683C88.5836 37.9851 93.0058 38.5083 97.8629 39.6004Z"
                                            fill="#FAFAFA"
                                        ></path>
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                                            d="M77.1387 78.7385C77.1387 82.1982 79.9448 85.0043 83.4046 85.0043C86.8643 85.0043 89.667 82.1982 89.667 78.7385C89.667 75.2788 86.8643 72.4727 83.4046 72.4727C79.9448 72.4727 77.1387 75.2788 77.1387 78.7385Z"
                                            fill="url(#paint1_linear_4664_62620)"
                                        ></path>
                                        <defs>
                                            <linearGradient
                                                id="paint0_linear_4664_62620"
                                                x1="97.394"
                                                y1="37.9475"
                                                x2="-45.6771"
                                                y2="112.385"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop stopColor="#EC796B"></stop>
                                                <stop offset="1" stopColor="#D672EF"></stop>
                                            </linearGradient>
                                            <linearGradient
                                                id="paint1_linear_4664_62620"
                                                x1="89.6626"
                                                y1="74.3429"
                                                x2="66.8299"
                                                y2="76.4834"
                                                gradientUnits="userSpaceOnUse"
                                            >
                                                <stop stopColor="#EC796B"></stop>
                                                <stop offset="1" stopColor="#D672EF"></stop>
                                            </linearGradient>
                                        </defs>
                                    </svg>
                                </div>
                            </div>
                            <div className="css-8hn81s">
                                <div className="css-14y9hkt">
                                    <div
                                        aria-valuemax="100"
                                        aria-valuemin="0"
                                        aria-valuenow="25.149979662698414"
                                        role="progressbar"
                                        className="css-1g2dxwf"
                                        style={"width: 25.15%"}
                                    ></div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-7onblj">
                                    <div className="css-0">
                                        <div
                                            id="popover-trigger-:rd:"
                                            aria-haspopup="dialog"
                                            aria-expanded="false"
                                            aria-controls="popover-content-:rd:"
                                            className="css-1xzdk2b"
                                        >
                                            In progress
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-6xwb09">
                                    Oct 03 through Oct 10
                                </div>
                            </div>
                            <div className="css-8hn81s">
                                <div className="css-p5pt5c">
                                    <div
                                        aria-valuemax="100"
                                        aria-valuemin="0"
                                        aria-valuenow="0"
                                        role="progressbar"
                                        className="css-1g2dxwf"
                                        style={{ width: "0%" }}
                                    ></div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-ryjd5e">
                                    <span
                                        id="popover-trigger-:rf:"
                                        aria-haspopup="dialog"
                                        aria-expanded="false"
                                        aria-controls="popover-content-:rf:"
                                    >
                                        Not started
                                    </span>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-6xwb09">
                                    Oct 10 through Oct 17
                                </div>
                            </div>
                            <div className="css-8hn81s">
                                <div className="css-p5pt5c">
                                    <div
                                        aria-valuemax="100"
                                        aria-valuemin="0"
                                        aria-valuenow="0"
                                        role="progressbar"
                                        className="css-1g2dxwf"
                                        style={{ width: "0%" }}
                                    ></div>
                                </div>
                            </div>
                            <div className="css-0">
                                <div className="chakra-text css-ryjd5e">
                                    <span
                                        id="popover-trigger-:rh:"
                                        aria-haspopup="dialog"
                                        aria-expanded="false"
                                        aria-controls="popover-content-:rh:"
                                    >
                                        Not started
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Defi;
